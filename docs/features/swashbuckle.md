...
This registers `ErrorResponseOperationFilter`, which implements `IOperationFilter`. It automatically adds error response schemas (400, 404, 500 by default) to all API operations in the Swagger document.
...
- Uses `ErrorResponseSchemaGenerator` internally (shared with the OpenAPI package)
...
For projects targeting .NET 9 or later, use the [OpenAPI package](openapi.md) instead. It integrates with the built-in `Microsoft.AspNetCore.OpenApi` support and does not depend on Swashbuckle.
