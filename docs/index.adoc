...
[source,bash,indent=0]
.FluentValidation integration (.NET 6-10)
----
...
The `ErrorLens.ErrorHandling.FluentValidation` satellite package adds automatic handling of `FluentValidation.ValidationException`.
...
Register it alongside the core library:
...
When a `FluentValidation.ValidationException` is thrown, the library produces a structured response identical in shape to DataAnnotations validation errors.
By default, only `Error` severity failures are included.
To include other severities:
...
===== Built-in error code mappings

The handler maps 14 built-in FluentValidation validators to `DefaultErrorCodes` constants:

[cols="2,2"]
|===
|FluentValidation Validator |Mapped Error Code
...
Custom error codes set via `.WithErrorCode("CUSTOM_CODE")` are preserved as-is.
Nested property names (e.g., `Address.ZipCode`) are converted with segment-by-segment camelCase (e.g., `address.zipCode`).
...
[cols="2,2"]
|===
|Strategy |Example (`UserNotFoundException`)

|`AllCaps` (default)
|`USER_NOT_FOUND`

|`FullQualifiedName`
|`MyApp.Exceptions.UserNotFoundException`
...
==== Customizing built-in handler messages

The default messages returned by built-in handlers can be overridden using the `BuiltInMessages` dictionary, without writing replacement handler classes:
...
The available keys correspond to `DefaultErrorCodes` constants used by the built-in handlers.
...
The fallback message is configurable via the `FallbackMessage` property:
...
Several configuration properties are validated at application startup:

* **JsonFieldNames**: Null or empty values are rejected; duplicate field names are detected and reported
* **ProblemDetailTypePrefix**: Must be a valid absolute URI or empty
...
The `retryAfter` field name in the response body can be customized via `JsonFieldNames`:
...
|Determines how an Exception is converted into a `code`. One of: `AllCaps`, `FullQualifiedName`, `KebabCase`, `PascalCase`, `DotSeparated`.
|`AllCaps`
