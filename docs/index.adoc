...
.PackageReference
----
<PackageReference Include="ErrorLens.ErrorHandling" Version="LATEST_VERSION_HERE" />
----
...
By default, the error code for `[Required]` is `REQUIRED`.
...
* The `code` is always `VALIDATION_ERROR` (unless there was an override defined).
...
You can register multiple customizer beans in your application.
...
This behaviour can be changed to log either nothing at all (`NoLogging`), or to log full stack traces (`WithStacktrace`) via the `ExceptionLogging` property.
...
|ExceptionLogging
|Controls how the exception should be logged. One of: `NoLogging`, `MessageOnly`, `WithStacktrace`.
|`MessageOnly`
...
The repository includes three sample projects:
...
If you have problems with the library, or you are missing a feature, feel free to create an issue on GitHub at https://github.com/AhmedV20/errorLens-errorhandling.
